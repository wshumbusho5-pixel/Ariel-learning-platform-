...
const GoogleLoginButton = dynamic(() => import('./GoogleLoginButton'), { ssr: false });
...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  const handleGoogleSuccess = async (accessToken: string) => {
    ...
    setError('');
    try {
      const response = await authAPI.oauthLogin('google', accessToken);
      localStorage.setItem('auth_token', response.access_token);
      onSuccess(response.user, response.access_token);
      onClose();
    } catch (err: any) {
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-full px-4 py-2.5 border border-[#2f3336] rounded-xl hover:bg-zinc-900 transition-colors text-[14px] font-medium"
            style={{ color: '#e7e9ea' }}
          >

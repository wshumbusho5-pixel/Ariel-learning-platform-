...
export default function FollowersPage() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    socialAPI.getFollowers(user.id as string)
      ...
      <div className="min-h-screen bg-black pb-24 lg:pl-[72px]">
        <div className="sticky top-0 z-40 bg-black border-b border-[#2f3336]">
          <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
            ...
            <h1 className="text-[17px] font-bold" style={{ color: '#e7e9ea' }}>Followers</h1>
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {!loading && people.length === 0 && (
            <div className="py-24 text-center px-8">
              ...
              <p className="text-sm font-semibold" style={{ color: '#e7e9ea' }}>No followers yet</p>
              <p className="text-xs mt-1" style={{ color: '#8b9099' }}>Share your profile to get your first follower.</p>
            </div>
          )}
            ...
            <div className="divide-y divide-zinc-800">
              {people.map(person => (
                <div key={person.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  >
                    {person.is_following ? 'Following' : 'Follow Back'}
                  </button>
                </div>
              ))}
            </div>
